...
check_sys(){
	if [[ -f /etc/redhat-release ]]; then
		...
		release="ubuntu"
	elif cat /etc/issue | grep -q -E -i "almalinux|Red Hat|redhat"; then
		release="almalinux"
	elif cat /proc/version | grep -q -E -i "debian"; then
		release="debian"
	elif cat /proc/version | grep -q -E -i "ubuntu"; then
		release="ubuntu"
	elif cat /proc/version | grep -q -E -i "almalinux|Red Hat|redhat"; then
		release="almalinux"
    fi
	bit=`uname -m`
}
...
Service_SSR(){
	if [[ ${release} = "almalinux" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/gexqin/shadowsocksr/master/ssr -O /etc/init.d/ssr; then
...
Check_Libsodium_ver(){
	...
	Libsodiumr_ver=`wget -qO- https://download.libsodium.org/libsodium/releases/LATEST.tar.gz | grep "<title>" | perl -e 'while($_=<>){ /Release (.*) · jedisct1/; print $1;}'`
...
centos_View_user_connection_info(){
	if [[ "${now_mode}" = "null" ]]; then
		now_mode="单端口" && user_total="1"
		IP_total=`netstat -anp |grep 'ESTABLISHED' |grep 'python' |grep 'tcp6' | grep '::ffff:' |awk '{print $4}' |sort -u |wc -l`
		user_port=`${jq_file} '.server_port' ${config_user_file}`
		user_IP=`netstat -anp |grep 'ESTABLISHED' |grep 'python' |grep 'tcp6' |grep "${user_port}" | grep '::ffff:' |awk '{print $5}' |awk -F ":" '{print $4}' |sort -u`
		user_IP_total=`netstat -anp |grep 'ESTABLISHED' |grep 'python' |grep 'tcp6' |grep "${user_port}" | grep '::ffff:' |awk '{print $5}' |awk -F ":" '{print $4}' |sort -u |wc -l`
		user_list_all="端口: ${Green_font_prefix}"${user_port}"${Font_color_suffix}, 链接IP总数: ${Green_font_prefix}"${user_IP_total}"${Font_color_suffix}, 当前链接IP: ${Green_font_prefix}"${user_IP}"${Font_color_suffix}\n"
		echo -e "当前模式: ${Green_font_prefix} "${now_mode}" ${Font_color_suffix}"
		echo -e ${user_list_all}
	else
...
Install_BBR(){
	[[ ${release} = "almalinux" ]] && echo -e "${Error} 本脚本不支持 CentOS系统安装 BBR !" && exit 1
	BBR_installation_status
	bash bbr.sh
}
Start_BBR(){
...
check_sys
[[ ${release} != "debian" ]] && [[ ${release} != "ubuntu" ]] && [[ ${release} != "almalinux" ]] && echo -e "${Error} 本脚本不支持当前系统 ${release} !" && exit 1
